...
FileStreaming::FileStreaming(ThreadData *tdp)
{
	...
	for(;;)
	{
		...
		if (tdp->fd != socket_fd)	// new seek request or start
		{
			if (socket_fd != 0)		// sockes change or first socket
			{
				close (socket_fd);
			}
			...
			if(http_range > 0)
			{
				Util::vlog("FileStreaming: performing http byte range seek");
			...
			else
			{
				if(byte_offset > 0)
				{
					Util::vlog("FileStreaming: performing byte_offset seek");
				...
				else
				{
					if(pct_offset > 0)
					{
						Util::vlog("FileStreaming: performing pct_offset seek");
					...
					else
					{
						...
						{
							Util::vlog("FileStreaming: performing startfrom seek");
			...
			http_reply += "\r\n";
			//Util::vlog("FileStreaming: httpanswer %s", http_reply.c_str());
		...
		{
			Util::vlog("FileStreaming: poll error");
		}
